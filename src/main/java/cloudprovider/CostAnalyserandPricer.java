	...
	public void tick(long fires) {
		...
		while (it.hasNext()) {
			...
			if (!vmu.isStarted()) {
				...
				totalEarnings += service.getCloudpricing().getPerTickQuote(vmu.rc) * ( ((vmu.getCompleteUsage() % service.getCloudpricing().getPeriodInTick())+1)*service.getCloudpricing().periodInTick  );
